<?php
    echo '
    <!-- Navigation -->
    <nav class="navbar navbar-expand-lg navbar-light fixed-top" id="mainNav">
    <div class="container">
        <a class="navbar-brand" href="ClientHome.php"><img src="images/logo.png" width="50" id="logo"></a>
        <a class="navbar-brand js-scroll-trigger" href="ClientHome.php" id="company">FoodEdge</a>
        <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarSupportedContent" aria-controls="#navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
            <span class="navbar-toggler-icon"></span>
        </button>
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            <ul class="navbar-nav ml-auto">
                <li class="nav-item"><a class="nav-link" href="OTCateringPackage1.php">Menu</a></li>
                <li class="nav-item"><a class="nav-link" href="OTFoodAndBeverages.php">Edit</a></li>
                <li class="nav-item"><a class="nav-link" href="OTUpdateOrders.php">Order</a></li>
                <li class="nav-item"><a class="nav-link" href="OTTransactions.php">Transactions</a></li>
                <li class="nav-item"><a class="nav-link" href="All_user_type_login_page.php"><i class="fa fa-user" style="margin-right: 7px;"></i>Logout</a></li></li>
            </ul>
        </div>
    </div>
    </nav>
    '
    
?>
